...
class YahooFinanceClient @Inject constructor(
    ...
    suspend fun fetchSymbolData(ticker: String): Result<Meta> {
        return try {
            val response: YahooChartResponse = httpClient.get("https://query1.finance.yahoo.com/v8/finance/chart/$ticker") {
            ...
            if (error != null) {
                return Result.failure(Exception("Yahoo API Error: ${error.code} - ${error.description}"))
            }
