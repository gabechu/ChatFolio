...
class PortfolioManagerTest {
    ...
    @Test
    fun `getLiveHoldings ignores holdings with zero shares`() = runTest {
    ...
    @Test
    fun `getLiveHoldings calculates profit correctly when Yahoo returns new data`() = runTest {
        val dummyPortfolio = PortfolioEntity(id = 1, name = "Default")
        coEvery { portfolioDao.getDefaultPortfolio() } returns dummyPortfolio
        ...
        assertThat(tslaResult.gainLoss).isEqualTo(500.0) // 2000 - 1500
        assertThat(tslaResult.gainLossPercent).isWithin(0.01).of(33.33) // (500 / 1500) * 100
    ...
    @Test
    fun `getLiveHoldings uses cache and avoids DB write when Yahoo returns same date`() = runTest {
        val dummyPortfolio = PortfolioEntity(id = 1, name = "Default")
        coEvery { portfolioDao.getDefaultPortfolio() } returns dummyPortfolio
        ...
        val yahooMeta = Meta(
            currency = "USD",
            symbol = "AAPL",
            regularMarketPrice = 185.0, // Let's pretend Yahoo drifted slightly during close, but date is same
            previousClose = 180.0,
            regularMarketTime = fakeTime
        )
        coEvery { yahooFinanceClient.fetchSymbolData("AAPL") } returns Result.success(yahooMeta)
    ...
    @Test
    fun `getGlobalSummary accurately converts currency using live FX rate`() = runTest {
        val dummyPortfolio = PortfolioEntity(id = 1, name = "Default")
        coEvery { portfolioDao.getDefaultPortfolio() } returns dummyPortfolio
